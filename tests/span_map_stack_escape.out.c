...
#define __CMINUS_DEFAULT_SELECT_Span_map_from_Data(_1, _2, _3, _4, NAME, ...) NAME
#define __CMINUS_DEFAULT_Span_map_from_Data_3(data, bytes, len) Span_map_from_Data(data, bytes, len, 0)
#define __CMINUS_DEFAULT_Span_map_from_Data_4(data, bytes, len, allow_raw) Span_map_from_Data(data, bytes, len, allow_raw)
#define Span_map_from_Data(...) __CMINUS_DEFAULT_SELECT_Span_map_from_Data(__VA_ARGS__, __CMINUS_DEFAULT_Span_map_from_Data_4, __CMINUS_DEFAULT_Span_map_from_Data_3)(__VA_ARGS__)
struct Span_Data Span_map_from_Data(void* data, int bytes, int len, int allow_raw);
struct Data* Span_ptr_at_Data(struct Span_Data* self, int index, const char* file, int line);
...
struct Span_Data make_view(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    int raw[2] = { 1, 2 };
    struct Span_Data values = ({ cminus_stack_note_caller_range(raw, sizeof(raw)); Span_map_from_Data(raw, sizeof(raw), 1, 0); });
...
{    char __cminus_stack_anchor;
    ...
    struct Span_Data values = make_view();

    __typeof__(((*Span_ptr_at_Data(&(values), 0, "tests/span_map_stack_escape.c-", 20)).a)) __cminus_return7 = ((*Span_ptr_at_Data(&(values), 0, "tests/span_map_stack_escape.c-", 20)).a);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return7;
}
#undef Span_map_from_Data
struct Span_Data Span_map_from_Data(void* data, int bytes, int len, int allow_raw){
    struct Span_Data out;
    unsigned long stack_id = 0UL;
    unsigned long align = (unsigned long)__alignof__(struct Data);
    ...
    if (sizeof(struct Data) == 0 || bytes % (int)sizeof(struct Data) != 0) {
        cminus_panic("span map byte size is not aligned to element size", __FILE__, __LINE__);
    }
    need = len * (int)sizeof(struct Data);
    if (len != 0 && need / len != (int)sizeof(struct Data)) {
    ...
    out.data = (struct Data*)data;
    out.len = len;
    out.base = (struct Data*)data;
    out.cap = bytes / (int)sizeof(struct Data);
