...
int cminus_control_ast_probe(int limit)
{
    int total = 0;
    ...
    for (int ignored = 0; ignored < 1; ignored += 1) {
        continue;
    }
    do {
        total += 1;
...
{    char __cminus_stack_anchor;
    ...
    __typeof__((cminus_control_ast_probe(4) == 10 ? 0 : 1)) __cminus_return6 = (cminus_control_ast_probe(4) == 10 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
