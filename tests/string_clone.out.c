...
{    char __cminus_stack_anchor;
    ...
    char* source;
    source = cminus_string_format("Alice");
    struct Person *person = cminus_gc_calloc(1, sizeof(struct Person));    void* __owned_old8 = person->name;


    person->name =({ char* __right_value_src6 = source; char* __right_value7 = NULL; if (__right_value_src6 != NULL) { __right_value7 = cminus_gc_calloc(strlen(__right_value_src6) + 1, sizeof(char)); strncpy(__right_value7, __right_value_src6, strlen(__right_value_src6) + 1); } __right_value7; });
    ...
    person->age = 42;
    struct Person *copy =({ struct Person* __right_value_src9 = person; struct Person* __right_value10 = NULL; if (__right_value_src9 != NULL) { __right_value10 = Person_clone(__right_value_src9); } __right_value10; });
    ...
    if (copy->name == person->name) {
        __typeof__((2)) __cminus_return12 = (2);
        if (copy != NULL) {
            Person_finalize(copy);
            cminus_gc_free(copy);
        }
    ...
    if (!cminus_string_eq(copy->name, "Alice") || copy->age != 42) {
        __typeof__((3)) __cminus_return13 = (3);
        if (copy != NULL) {
            Person_finalize(copy);
            cminus_gc_free(copy);
        }
