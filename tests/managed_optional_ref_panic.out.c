...
enum {
    Optional_Ref_int_TAG_Some,
    Optional_Ref_int_TAG_None
};
static __attribute__((unused)) struct Optional_Ref_int Optional_Ref_int_Some(struct Ref_int value)
{
    struct Optional_Ref_int out = {0};
    out.tag = Optional_Ref_int_TAG_Some;
    out.origin_kind = 0UL;
    out.origin_stack_id = 0UL;
    out.payload.Some = value;
    return out;
}
static __attribute__((unused)) int Optional_Ref_int_is_Some(struct Optional_Ref_int* self)
{
    return self->tag == Optional_Ref_int_TAG_Some;
}
static __attribute__((unused)) struct Ref_int Optional_Ref_int_get_Some(struct Optional_Ref_int* self)
{
    return self->payload.Some;
}
static __attribute__((unused)) struct Optional_Ref_int Optional_Ref_int_None(void)
{
    struct Optional_Ref_int out = {0};
    out.tag = Optional_Ref_int_TAG_None;
    out.origin_kind = 0UL;
    out.origin_stack_id = 0UL;
    return out;
}
static __attribute__((unused)) int Optional_Ref_int_is_None(struct Optional_Ref_int* self)
{
    return self->tag == Optional_Ref_int_TAG_None;
}
#define __CMINUS_DEFAULT_SELECT_Ref_from_int(_1, _2, NAME, ...) NAME
#define __CMINUS_DEFAULT_Ref_from_int_1(data) Ref_from_int(data, 0)
#define __CMINUS_DEFAULT_Ref_from_int_2(data, allow_raw) Ref_from_int(data, allow_raw)
#define Ref_from_int(...) __CMINUS_DEFAULT_SELECT_Ref_from_int(__VA_ARGS__, __CMINUS_DEFAULT_Ref_from_int_2, __CMINUS_DEFAULT_Ref_from_int_1)(__VA_ARGS__)
struct Ref_int Ref_from_int(int* data, int allow_raw);
int Ref_get_int(struct Ref_int* self);
...
static __attribute__((unused)) struct ManagedData* ManagedData_clone(struct ManagedData* self)
{
    struct ManagedData* copy = cminus_gc_calloc(1, sizeof(struct ManagedData));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
{    char __cminus_stack_anchor;
    ...
    struct ManagedData *owner = cminus_gc_calloc(1, sizeof(struct ManagedData));
    struct Ref_int reference = Ref_from_int(&owner->value, 0);
    struct Optional_Ref_int maybe = Optional_Ref_int_Some(reference);
    struct Ref_int unwrapped = Optional_Ref_int_get_Some(&maybe);

    owner->value = 17;
    if (Ref_get_int(&unwrapped) != 17) {
        __typeof__((1)) __cminus_return6 = (1);
        if (owner != NULL) {
            cminus_gc_free(owner);
        }
    ...
    __typeof__((Ref_get_int(&unwrapped))) __cminus_return7 = (Ref_get_int(&unwrapped));
    if (owner != NULL) {
        cminus_gc_free(owner);
    }
...
struct Ref_int Ref_from_int(int* data, int allow_raw){
    ...
    if (allow_raw) {
        ...
        cminus_stack_note_caller_range(data, sizeof(int));
