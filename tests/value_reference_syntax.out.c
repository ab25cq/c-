...
int inspect(const struct Data *data)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    __typeof__((data->value)) __cminus_return6 = (data->value);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return6;
}

void update(struct Data *data, int value)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    data->value = value;    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    struct Data *boxed = ({ struct Data* __right_value7 = cminus_gc_calloc(1, sizeof(struct Data)); if (__right_value7 != NULL) { __right_value7->value = 7; } __right_value7; });
    struct Data *later = {0};
    memset(&later, 0, sizeof(later));


    update(&local, 5);    void* __owned_old9 = later;

    later = ({ struct Data* __right_value8 = cminus_gc_calloc(1, sizeof(struct Data)); if (__right_value8 != NULL) { __right_value8->value = 9; } __right_value8; });
    ...
    if (inspect(&local) != 5) {
        ...
        if (later != NULL) {
            cminus_gc_free(later);
        }

        if (boxed != NULL) {
            cminus_gc_free(boxed);
        }

        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return10;
    }
    __typeof__((inspect(boxed) == 7 && inspect(later) == 9 ? 0 : 1)) __cminus_return11 = (inspect(boxed) == 7 && inspect(later) == 9 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
