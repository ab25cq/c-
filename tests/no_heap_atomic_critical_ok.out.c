...
void Atomic_store_unsigned_int(struct Atomic_unsigned_int* self, unsigned int value);
...
{    char __cminus_stack_anchor;
    ...
    struct Atomic_unsigned_int value = Atomic_init_unsigned_int(0u);
    struct Critical guard = Critical_enter();

    Atomic_store_unsigned_int(&value, 42u);
    Critical_leave(&guard);
    __typeof__((Atomic_load_unsigned_int(&value) == 42u ? 0 : 1)) __cminus_return6 = (Atomic_load_unsigned_int(&value) == 42u ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
