...
static __attribute__((unused)) struct PageBits* PageBits_clone(struct PageBits* self)
{
    struct PageBits* copy = cminus_gc_calloc(1, sizeof(struct PageBits));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
{    char __cminus_stack_anchor;
    ...
    struct PageBits pages = {0};
    memset(&pages, 0, sizeof(pages));

    Bitmap map = {0};
    memset(&map, 0, sizeof(map));

    struct __CMinusIndex_int slot = {0};
    memset(&slot, 0, sizeof(slot));


    map = ({ cminus_stack_note_caller_range(pages.words, sizeof(pages.words)); Bitmap_from(pages.words, (int)(sizeof(pages.words) * 8)); });
    if (Bitmap_len(&map) != sizeof(pages.words) * 8 || Bitmap_word_len(&map) != 1) {
        __typeof__((1)) __cminus_return6 = (1);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return6;
    }
    Bitmap_set(&map, 3);
    if (!Bitmap_test(&map, 3)) {
        __typeof__((2)) __cminus_return7 = (2);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return7;
    }
    Bitmap_clear_bit(&map, 3);
    if (Bitmap_test(&map, 3)) {
        __typeof__((3)) __cminus_return8 = (3);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return8;
    }
    slot = Bitmap_alloc_opt(&map);
    if (__CMinusIndex_int_is_None(&slot) || __CMinusIndex_int_get_Some(&slot) != 0 || !Bitmap_test(&map, 0)) {
        __typeof__((4)) __cminus_return9 = (4);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return9;
    }
    Bitmap_free_bit(&map, 0);
    if (Bitmap_test(&map, 0)) {
        __typeof__((5)) __cminus_return10 = (5);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
