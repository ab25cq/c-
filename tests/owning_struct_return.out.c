...
static void Message_finalize(struct Message* self)
{
    if (self == NULL) {
        return;
    }
    if (self->text != NULL) {
        cminus_gc_free(self->text);
    }

}


static __attribute__((unused)) struct Message* Message_clone(struct Message* self)
{
    struct Message* copy = cminus_gc_calloc(1, sizeof(struct Message));
    if (copy == NULL || self == NULL) {
        return copy;
    }
    if (self->text != NULL) {
        copy->text = cminus_gc_calloc(strlen(self->text) + 1, sizeof(char));
...
struct Message make_message(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);
    ...
    char* temporary;
    temporary = cminus_string_format("temporary");    void* __owned_old6 = message.text;
    message.text = cminus_string_format("hello");
    if (__owned_old6 != NULL) {
        cminus_gc_free(__owned_old6);
    }


    if (cminus_string_len(temporary) != 9) {    void* __owned_old7 = message.text;
        message.text = cminus_string_format("bad");
    ...
    __typeof__((message)) __cminus_return8 = (message);
    if (temporary != NULL) {
        cminus_gc_free(temporary);
    }
...
{    char __cminus_stack_anchor;
    ...
    struct Message message = make_message();

    __typeof__((cminus_string_eq(message.text, "hello") ? 0 : 1)) __cminus_return9 = (cminus_string_eq(message.text, "hello") ? 0 : 1);
