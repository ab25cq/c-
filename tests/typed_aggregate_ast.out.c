...
struct TypedAggregate {
    int value;
};

static __attribute__((unused)) struct TypedAggregate* TypedAggregate_clone(struct TypedAggregate* self)
{
    struct TypedAggregate* copy = cminus_gc_calloc(1, sizeof(struct TypedAggregate));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
int cminus_aggregate_ast_probe(void)
{
    struct TypedAggregate item = {0};
    memset(&item, 0, sizeof(item));

    enum TypedKind kind = TYPED_KIND_ONE;
...
{    char __cminus_stack_anchor;
    ...
    __typeof__((cminus_aggregate_ast_probe() == 1 ? 0 : 1)) __cminus_return6 = (cminus_aggregate_ast_probe() == 1 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
