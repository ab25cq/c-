...
{    char __cminus_stack_anchor;
    ...
    struct Span_int bad = {0};
    memset(&bad, 0, sizeof(bad));


    data[0] = 1;
    data[1] = 2;
    values = ({ cminus_stack_note_caller_range(data, sizeof(data)); Span_from_int(data, 2, 0); });
    bad = Span_offset_int(&values, 3, "tests/span_operator_panic.c-", 10);
    __typeof__((Span_len_int(&bad))) __cminus_return6 = (Span_len_int(&bad));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
