...
#define __CMINUS_DEFAULT_SELECT_FixedVec_from_bytes_int(_1, _2, _3, NAME, ...) NAME
#define __CMINUS_DEFAULT_FixedVec_from_bytes_int_2(data, bytes) FixedVec_from_bytes_int(data, bytes, 0)
#define __CMINUS_DEFAULT_FixedVec_from_bytes_int_3(data, bytes, allow_raw) FixedVec_from_bytes_int(data, bytes, allow_raw)
#define FixedVec_from_bytes_int(...) __CMINUS_DEFAULT_SELECT_FixedVec_from_bytes_int(__VA_ARGS__, __CMINUS_DEFAULT_FixedVec_from_bytes_int_3, __CMINUS_DEFAULT_FixedVec_from_bytes_int_2)(__VA_ARGS__)
struct FixedVec_int FixedVec_from_bytes_int(int* data, int bytes, int allow_raw);
int FixedVec_push_int(struct FixedVec_int* self, int value);
int FixedVec_get_int(struct FixedVec_int* self, int index);
...
struct FixedVec_int make_vec(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    int raw[2] = { 1, 2 };
    struct FixedVec_int values = ({ cminus_stack_note_caller_range(raw, sizeof(raw)); FixedVec_from_bytes_int(raw, sizeof(raw), 0); });

    FixedVec_push_int(&values, 1);
...
{    char __cminus_stack_anchor;
    ...
    struct FixedVec_int values = make_vec();

    __typeof__((FixedVec_get_int(&values, 0))) __cminus_return7 = (FixedVec_get_int(&values, 0));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
