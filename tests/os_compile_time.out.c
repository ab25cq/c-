...
static __attribute__((unused)) struct TrapFrame* TrapFrame_clone(struct TrapFrame* self)
{
    struct TrapFrame* copy = cminus_gc_calloc(1, sizeof(struct TrapFrame));
    if (copy == NULL || self == NULL) {
        return copy;
    }
    copy->r0 = self->r0;
...
_Static_assert(sizeof(struct TrapFrame) == 12, "trap frame size");
_Static_assert(__builtin_offsetof(struct TrapFrame, pc) == 8, "pc offset");
...
{    char __cminus_stack_anchor;
    ...
    _Static_assert(__builtin_offsetof(struct TrapFrame, r1) == 4, "r1 offset");
    __typeof__((__builtin_offsetof(struct TrapFrame, pc))) __cminus_return6 = (__builtin_offsetof(struct TrapFrame, pc));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
