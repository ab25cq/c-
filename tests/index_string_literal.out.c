...
int main(void)
{
    ...
    Vec_push_int(&nums, 10);
    Vec_push_int(&nums, 20);

    /* The "a[0]" and 'x[1]' inside these literals must not be rewritten. */
    printf("a[0] x[1] = %d\n", ({ struct __CMinusIndex_int __index_result0 = Vec_get_opt_int(&nums, 1); if (__index_result0.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/index_string_literal.c-", 13); } __index_result0.payload.Some; }));
    printf("%c\n", 'a');
