...
{
    struct Child* copy = cminus_gc_calloc(1, sizeof(struct Child));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
{    char __cminus_stack_anchor;
    ...
    __typeof__((child->value)) __cminus_return6 = (child->value);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    struct Parent *parent = cminus_gc_calloc(1, sizeof(struct Parent));
    struct Child *child = cminus_gc_calloc(1, sizeof(struct Child));

    child->value = 11;    void* __owned_old7 = parent->child;
    ...
    parent->count = child_value(parent->child) + 1;
