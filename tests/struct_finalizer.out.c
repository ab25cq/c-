...
{    char __cminus_stack_anchor;
    ...
    struct Holder *stack = cminus_gc_calloc(1, sizeof(struct Holder));

    struct Holder *heap = cminus_gc_calloc(1, sizeof(struct Holder));

    __typeof__((stack != NULL && heap != NULL ? 0 : 1)) __cminus_return6 = (stack != NULL && heap != NULL ? 0 : 1);
    if (heap != NULL) {
