...
unsigned int Volatile_read_unsigned_int(struct Volatile_unsigned_int* self);
...
{    char __cminus_stack_anchor;
    ...
    unsigned int raw = 0u;
    struct Volatile_unsigned_int cell = {0};
    memset(&cell, 0, sizeof(cell));
 {
        cell = Volatile_from_addr_unsigned_int((unsigned long)&raw);
    }
    Volatile_write_unsigned_int(&cell, 42u);
    __typeof__((Volatile_read_unsigned_int(&cell) == 42u && raw == 42u ? 0 : 1)) __cminus_return6 = (Volatile_read_unsigned_int(&cell) == 42u && raw == 42u ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
