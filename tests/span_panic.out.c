...
{    char __cminus_stack_anchor;
    ...
    struct Span_int values = ({ cminus_stack_note_caller_range(data, sizeof(data)); Span_from_int(data, 1, 0); });

    data[0] = 1;
    __typeof__(((*Span_ptr_at_int(&(values), 2, "tests/span_panic.c-", 7)))) __cminus_return6 = ((*Span_ptr_at_int(&(values), 2, "tests/span_panic.c-", 7)));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
