...
{    char __cminus_stack_anchor;
    ...
    Vec_push_int(nums, 2);
    Vec_push_int(nums, 3);
    Vec_push_Item_ptr(item_vec, ({ struct Item* __right_value6 = cminus_gc_calloc(1, sizeof(struct Item)); if (__right_value6 != NULL) { __right_value6->value = 4; } __right_value6; }));
    Vec_push_Item_ptr(item_vec, ({ struct Item* __right_value7 = cminus_gc_calloc(1, sizeof(struct Item)); if (__right_value7 != NULL) { __right_value7->value = 5; } __right_value7; }));
    ...
    if (({ struct __CMinusIndex_int __index_result0 = Vec_get_opt_int(nums, 1); if (__index_result0.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/generics_foreach.c-", 41); } __index_result0.payload.Some; }) != 2) {
        __typeof__((1)) __cminus_return9 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (Vec_len_int(dyn) != 2 || Vec_capacity_int(dyn) < 2 || Vec_last_int(dyn) != 9) {
        __typeof__((1)) __cminus_return11 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!Vec_set_int(dyn, 0, 10) || Vec_get_int(dyn, 0) != 10) {
        __typeof__((1)) __cminus_return12 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!__CMinusIndex_int_is_Some(&popped_vec) || __CMinusIndex_int_get_Some(&popped_vec) != 9 || Vec_len_int(dyn) != 1) {
        __typeof__((1)) __cminus_return13 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!Vec_is_empty_int(dyn)) {
        __typeof__((1)) __cminus_return14 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (({ struct __CMinusIndex_int __index_result1 = List_get_opt_int(list, 1); if (__index_result1.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/generics_foreach.c-", 67); } __index_result1.payload.Some; }) != 7) {
        __typeof__((1)) __cminus_return16 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!__CMinusIndex_int_is_Some(&map_value) || __CMinusIndex_int_get_Some(&map_value) != 200 || !Map_contains_int_int(map, 101)) {
        __typeof__((1)) __cminus_return17 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (Map_contains_int_int(map, 100) || Map_len_int_int(map) != 1) {
        __typeof__((1)) __cminus_return18 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (List_len_int(list) != 3 || List_first_int(list) != 5 || List_last_int(list) != 7) {
        __typeof__((1)) __cminus_return19 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!List_set_int(list, 1, 66) || List_get_int(list, 1) != 66) {
        __typeof__((1)) __cminus_return20 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
    ...
    if (!__CMinusIndex_int_is_Some(&front) || __CMinusIndex_int_get_Some(&front) != 5 || List_len_int(list) != 2) {
        __typeof__((1)) __cminus_return21 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
        ...
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return21;
    }
    List_clear_int(list);
    if (!List_is_empty_int(list)) {
        __typeof__((1)) __cminus_return22 = (1);
        if (map != NULL) {
            Map_delete_int_int(map);
            cminus_gc_free(map);
        }
        ...
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return22;
    }
    __typeof__((sum == 6 && item_sum == 9 && list_sum == 13 ? 0 : 1)) __cminus_return23 = (sum == 6 && item_sum == 9 && list_sum == 13 ? 0 : 1);
    if (map != NULL) {
        Map_delete_int_int(map);
        cminus_gc_free(map);
    }
    ...
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return23;
}
struct Vec_int* Vec_new_int(void){
    return cminus_gc_calloc(1, sizeof(struct Vec_int));
}
struct Vec_Item_ptr* Vec_new_Item_ptr(void){
    return cminus_gc_calloc(1, sizeof(struct Vec_Item_ptr));
}
void Vec_push_int(struct Vec_int* self, int value){
    int* next;
...
void Vec_push_Item_ptr(struct Vec_Item_ptr* self, struct Item* value){
    ...
    if (self->len >= self->cap) {
        next = cminus_gc_calloc(next_cap, sizeof(struct Item*));
        if (next == NULL) {
            abort();
        }
        if (self->data != NULL && self->len > 0) {
            memcpy(next, self->data, sizeof(struct Item*) * self->len);
            cminus_gc_free(self->data);
        }
        self->data = next;
        self->cap = next_cap;
...
int Vec_reserve_int(struct Vec_int* self, int cap){
    ...
    next = cminus_gc_calloc(cap, sizeof(int));
    if (next == NULL) {
        return 0;
    }
    if (self->data != NULL && self->len > 0) {
