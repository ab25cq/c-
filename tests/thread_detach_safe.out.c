...
void Atomic_store_int(struct Atomic_int* self, int value);
...
struct Atomic_int detached_done;
...
{    char __cminus_stack_anchor;
    ...
    Atomic_store_int(&detached_done, 1);
...
{    char __cminus_stack_anchor;
    ...
    detached_done = Atomic_init_int(0);
    worker = Thread_spawn(detached_worker);
    Thread_detach(&worker);
    while (Atomic_load_int(&detached_done) == 0) {
        Thread_yield();
    }
