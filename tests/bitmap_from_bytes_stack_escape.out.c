...
Bitmap make_bitmap(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    unsigned long words[1] = { 0 };
    Bitmap bits = ({ cminus_stack_note_caller_range(words, sizeof(words)); Bitmap_from_bytes(words, sizeof(words)); });

    Bitmap_set(&bits, 0);
    __typeof__((bits)) __cminus_return6 = (bits);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    Bitmap bits = make_bitmap();

    __typeof__((Bitmap_test(&bits, 0))) __cminus_return7 = (Bitmap_test(&bits, 0));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
