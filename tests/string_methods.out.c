...
{    char __cminus_stack_anchor;
    ...
    char* text;
    text = cminus_string_format("hello");

    if (cminus_string_len(text) != 5) {
    ...
    if (!cminus_string_eq(text, "hello")) {
    ...
    if (cminus_string_cmp(text, "hello") != 0) {
    ...
    if (!cminus_string_contains(text, "ell")) {
    ...
    if (!cminus_string_starts_with(text, "he")) {
    ...
    if (!cminus_string_ends_with(text, "lo")) {
        __typeof__((6)) __cminus_return11 = (6);
        if (text != NULL) {
            cminus_gc_free(text);
        }
    ...
    if (({ char* __right_value13 = NULL; __right_value13 = cminus_string_format("abc"); int __right_value_cond14 = (!cminus_string_eq(__right_value13, "abc")) != 0; cminus_gc_free(__right_value13); __right_value_cond14; })) {
        __typeof__((7)) __cminus_return12 = (7);
        if (text != NULL) {
            cminus_gc_free(text);
        }
    ...
    __typeof__((0)) __cminus_return15 = (0);
    if (text != NULL) {
        cminus_gc_free(text);
    }
