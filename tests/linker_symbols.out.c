...
__attribute__((used, externally_visible)) char __kernel_start[1];
__attribute__((used, externally_visible)) char __kernel_end[1];

int main(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    unsigned long start = ((unsigned long)(__kernel_start));
    unsigned long end = ((unsigned long)(__kernel_end));

    __typeof__((start != 0u && end != 0u ? 0 : 1)) __cminus_return6 = (start != 0u && end != 0u ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
