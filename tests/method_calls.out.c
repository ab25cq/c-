...
{    char __cminus_stack_anchor;
    ...
    self->value = 42;    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    struct data *d = cminus_gc_calloc(1, sizeof(struct data));
    struct data *p = cminus_gc_calloc(1, sizeof(struct data));
    ...
    if (cminus_string_cmp("aaa", "aaa") != 0) {
    ...
    __typeof__((cminus_string_cmp("aaa", "aaa"))) __cminus_return8 = (cminus_string_cmp("aaa", "aaa"));
    if (p != NULL) {
        cminus_gc_free(p);
    }
