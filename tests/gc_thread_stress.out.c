...
struct Atomic_int Atomic_init_int(int value);
int Atomic_load_int(struct Atomic_int* self);
int Atomic_fetch_add_int(struct Atomic_int* self, int value);
...
struct Atomic_int gc_workers_done;
...
{    char __cminus_stack_anchor;
    ...
    while (i < 1000) { {
            ...
            cminus_ptr_require_alive(interior, kind, allocation_id, __FILE__, __LINE__);
            cminus_gc_free(allocation);
        }
        i++;
    }
    Atomic_fetch_add_int(&gc_workers_done, 1);
...
{    char __cminus_stack_anchor;
    ...
    gc_workers_done = Atomic_init_int(0);
    first = Thread_spawn(gc_stress_worker);
    second = Thread_spawn(gc_stress_worker);
    third = Thread_spawn(gc_stress_worker);
    fourth = Thread_spawn(gc_stress_worker);
    result += Thread_join(&first);
    result += Thread_join(&second);
    result += Thread_join(&third);
    result += Thread_join(&fourth);
    if (Atomic_load_int(&gc_workers_done) != 4) {
        __typeof__((2)) __cminus_return8 = (2);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return8;
    }
    __typeof__((result)) __cminus_return9 = (result);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return9;
}
struct Atomic_int Atomic_init_int(int value){
    struct Atomic_int out;
