...
struct StatementExpressionItem {
    int value;
};

static __attribute__((unused)) struct StatementExpressionItem* StatementExpressionItem_clone(struct StatementExpressionItem* self)
{
    struct StatementExpressionItem* copy = cminus_gc_calloc(1, sizeof(struct StatementExpressionItem));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
int cminus_statement_expression_ast_probe(struct StatementExpressionItem *item)
{
    return item->value;
}
...
{    char __cminus_stack_anchor;
    ...
    int result = cminus_statement_expression_ast_probe(
        ({ struct StatementExpressionItem* __right_value6 = cminus_gc_calloc(1, sizeof(struct StatementExpressionItem)); if (__right_value6 != NULL) { __right_value6->value = 5; } __right_value6; }));
    __typeof__((result == 5 ? 0 : 1)) __cminus_return7 = (result == 5 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
