...
#define CMINUS_TYPED_EXPANSION_VALUE 5
#include <string.h>
#include <stdlib.h>
#include <stdio.h>
#include <execinfo.h>
...
static __attribute__((unused)) struct ExpansionData* ExpansionData_clone(struct ExpansionData* self)
{
    struct ExpansionData* copy = cminus_gc_calloc(1, sizeof(struct ExpansionData));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
int cminus_expansion_ast_probe(void)
{
    struct ExpansionData data = {0};
    memset(&data, 0, sizeof(data));

    char* text;
    text = cminus_string_format("initial");

    data.value = CMINUS_TYPED_EXPANSION_VALUE;    void* __owned_old6 = text;
    text = cminus_string_format("updated");
    if (__owned_old6 != NULL) {
        cminus_gc_free(__owned_old6);
    }


    __typeof__((cminus_string_eq(text, "updated") ? data.value : 0)) __cminus_return7 = (cminus_string_eq(text, "updated") ? data.value : 0);
    if (text != NULL) {
        cminus_gc_free(text);
    }
...
{    char __cminus_stack_anchor;
    ...
    __typeof__((cminus_expansion_ast_probe() == 5 ? 0 : 1)) __cminus_return8 = (cminus_expansion_ast_probe() == 5 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
