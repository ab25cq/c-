...
static __attribute__((unused)) struct Data* Data_clone(struct Data* self)
{
    ...
    memcpy(copy->items, self->items, sizeof(copy->items));
    return copy;
}
...
{    char __cminus_stack_anchor;
    ...
    data.items[0] = 3;
    data.items[2] = 4;
    __typeof__((data.value == 7 && data.items[0] + data.items[2] == 7 ? 0 : 1)) __cminus_return6 = (data.value == 7 && data.items[0] + data.items[2] == 7 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
