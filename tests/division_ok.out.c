...
{    char __cminus_stack_anchor;
    ...
    int a = 10;
    int b = 2;
    int c = a / ({ __auto_type __divisor0 = (b); if (__divisor0 == 0) { cminus_panic("division by zero", "tests/division_ok.c-", 7); } __divisor0; });
    int d = a % ({ __auto_type __divisor1 = (3); if (__divisor1 == 0) { cminus_panic("division by zero", "tests/division_ok.c-", 8); } __divisor1; });
    printf("%d %d\n", c, d);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return c == 5 && d == 1 ? 0 : 1;
}