...
{    char __cminus_stack_anchor;
    ...
    int a = 10;
    int b = 2;
    int c = a / ({ __auto_type __divisor6 = (b); if (__divisor6 == 0) { cminus_panic("division by zero", "tests/division_ok.c-", 7); } __divisor6; });
    int d = a % ({ __auto_type __divisor7 = (3); if (__divisor7 == 0) { cminus_panic("division by zero", "tests/division_ok.c-", 8); } __divisor7; });
    printf("%d %d\n", c, d);
    __typeof__((c == 5 && d == 1 ? 0 : 1)) __cminus_return8 = (c == 5 && d == 1 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
