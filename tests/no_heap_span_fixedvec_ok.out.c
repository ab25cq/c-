...
#define __CMINUS_DEFAULT_SELECT_FixedVec_from_unsigned_char(_1, _2, _3, NAME, ...) NAME
#define __CMINUS_DEFAULT_FixedVec_from_unsigned_char_2(data, cap) FixedVec_from_unsigned_char(data, cap, 0)
#define __CMINUS_DEFAULT_FixedVec_from_unsigned_char_3(data, cap, allow_raw) FixedVec_from_unsigned_char(data, cap, allow_raw)
#define FixedVec_from_unsigned_char(...) __CMINUS_DEFAULT_SELECT_FixedVec_from_unsigned_char(__VA_ARGS__, __CMINUS_DEFAULT_FixedVec_from_unsigned_char_3, __CMINUS_DEFAULT_FixedVec_from_unsigned_char_2)(__VA_ARGS__)
struct FixedVec_unsigned_char FixedVec_from_unsigned_char(unsigned char* data, int cap, int allow_raw);
int FixedVec_len_unsigned_char(struct FixedVec_unsigned_char* self);
int FixedVec_push_unsigned_char(struct FixedVec_unsigned_char* self, unsigned char value);
struct __CMinusIndex_unsigned_char FixedVec_get_opt_unsigned_char(struct FixedVec_unsigned_char* self, int index);
...
static __attribute__((unused)) struct Packet* Packet_clone(struct Packet* self)
{
    struct Packet* copy = cminus_gc_calloc(1, sizeof(struct Packet));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
{    char __cminus_stack_anchor;
    ...
    struct Span_unsigned_char all = {0};
    memset(&all, 0, sizeof(all));

    struct FixedVec_unsigned_char used = {0};
    memset(&used, 0, sizeof(used));


    all = ({ cminus_stack_note_caller_range(pkt.bytes, sizeof(pkt.bytes)); Span_from_unsigned_char(pkt.bytes, 8, 0); });
    used = ({ cminus_stack_note_caller_range(pkt.bytes, sizeof(pkt.bytes)); FixedVec_from_unsigned_char(pkt.bytes, 8, 0); });
    FixedVec_push_unsigned_char(&used, 10);
    FixedVec_push_unsigned_char(&used, 20);
    if (Span_len_unsigned_char(&all) != 8 || FixedVec_len_unsigned_char(&used) != 2 || ({ struct __CMinusIndex_unsigned_char __index_result0 = FixedVec_get_opt_unsigned_char(&(used), 1); if (__index_result0.tag == __CMinusIndex_unsigned_char_TAG_None) { cminus_panic("index out of range", "tests/no_heap_span_fixedvec_ok.c-", 17); } __index_result0.payload.Some; }) != 20) {
...
#undef FixedVec_from_unsigned_char
struct FixedVec_unsigned_char FixedVec_from_unsigned_char(unsigned char* data, int cap, int allow_raw){
    struct FixedVec_unsigned_char out;
    unsigned long stack_id = 0UL;
...
struct __CMinusIndex_unsigned_char FixedVec_get_opt_unsigned_char(struct FixedVec_unsigned_char* self, int index){
    ...
    cminus_ptr_require_alive(self->data, self->origin_kind, self->origin_stack_id, __FILE__, __LINE__);
    return __CMinusIndex_unsigned_char_Some(self->data[index]);
}
