...
typedef int inline_c_value;

static inline int inline_c_mul(int value)
{
    return value * INLINE_C_SCALE;
}
...
{    char __cminus_stack_anchor;
    ...
    int local = 3;
    local = inline_c_mul(local);

    __typeof__((local == 12 ? 0 : 1)) __cminus_return6 = (local == 12 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
