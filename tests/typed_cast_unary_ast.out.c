...
int cminus_cast_unary_ast_probe(int value)
{
    int negative = -value;
    int inverse = ~value;
    int truth = !value;
    ...
    int alignment = 0; {
        converted = (int)value;
        bytes = (int)sizeof(value);
        alignment = (int)_Alignof(int);
    }

    return negative + inverse + truth + converted + bytes + alignment;
}
...
{    char __cminus_stack_anchor;
    ...
    __typeof__((cminus_cast_unary_ast_probe(1) == 6 ? 0 : 1)) __cminus_return6 = (cminus_cast_unary_ast_probe(1) == 6 ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
