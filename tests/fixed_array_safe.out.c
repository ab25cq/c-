...
#define __CMINUS_DEFAULT_SELECT_FixedVec_from_int(_1, _2, _3, NAME, ...) NAME
#define __CMINUS_DEFAULT_FixedVec_from_int_2(data, cap) FixedVec_from_int(data, cap, 0)
#define __CMINUS_DEFAULT_FixedVec_from_int_3(data, cap, allow_raw) FixedVec_from_int(data, cap, allow_raw)
#define FixedVec_from_int(...) __CMINUS_DEFAULT_SELECT_FixedVec_from_int(__VA_ARGS__, __CMINUS_DEFAULT_FixedVec_from_int_3, __CMINUS_DEFAULT_FixedVec_from_int_2)(__VA_ARGS__)
struct FixedVec_int FixedVec_from_int(int* data, int cap, int allow_raw);
int FixedVec_len_int(struct FixedVec_int* self);
int FixedVec_capacity_int(struct FixedVec_int* self);
int FixedVec_push_int(struct FixedVec_int* self, int value);
struct __CMinusIndex_int FixedVec_get_opt_int(struct FixedVec_int* self, int index);
struct Span_int FixedVec_as_span_int(struct FixedVec_int* self);
...
static __attribute__((unused)) struct Buffer* Buffer_clone(struct Buffer* self)
{
    struct Buffer* copy = cminus_gc_calloc(1, sizeof(struct Buffer));
    if (copy == NULL || self == NULL) {
        return copy;
    }
    memcpy(copy->data, self->data, sizeof(copy->data));
    return copy;
}
...
{    char __cminus_stack_anchor;
    ...
    struct Buffer *buf = cminus_gc_calloc(1, sizeof(struct Buffer));
    struct Span_int view = {0};
    memset(&view, 0, sizeof(view));

    struct Span_int used = {0};
    memset(&used, 0, sizeof(used));

    struct FixedVec_int values = {0};
    ...
    view = ({ cminus_stack_note_caller_range(buf->data, sizeof(buf->data)); Span_from_int(buf->data, 4, 0); });
    if (Span_len_int(&view) != 4 || (*Span_ptr_at_int(&(view), 2, "tests/fixed_array_safe.c-", 20)) != 30) {
    ...
    values = ({ cminus_stack_note_caller_range(buf->data, sizeof(buf->data)); FixedVec_from_int(buf->data, 4, 0); });
    FixedVec_push_int(&values, 7);
    FixedVec_push_int(&values, 8);
    FixedVec_push_int(&values, 9);
    if (FixedVec_len_int(&values) != 3 || FixedVec_capacity_int(&values) != 4) {
    ...
    if (({ struct __CMinusIndex_int __index_result0 = FixedVec_get_opt_int(&(values), 0); if (__index_result0.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/fixed_array_safe.c-", 31); } __index_result0.payload.Some; }) != 7 || ({ struct __CMinusIndex_int __index_result1 = FixedVec_get_opt_int(&(values), 2); if (__index_result1.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/fixed_array_safe.c-", 31); } __index_result1.payload.Some; }) != 9) {
    ...
    used = FixedVec_as_span_int(&values);
    if ((*Span_ptr_at_int(&(used), 1, "tests/fixed_array_safe.c-", 35)) != 8) {
        __typeof__((4)) __cminus_return9 = (4);
        if (buf != NULL) {
            cminus_gc_free(buf);
        }
...
#undef FixedVec_from_int
struct FixedVec_int FixedVec_from_int(int* data, int cap, int allow_raw){
    struct FixedVec_int out;
    unsigned long stack_id = 0UL;
    ...
    if (allow_raw) {
        ...
        cminus_stack_note_caller_range(data, (size_t)out.cap * sizeof(int));
        out.origin_kind = cminus_ptr_classify(data, &stack_id);
    }
    out.origin_stack_id = stack_id;
    return out;
...
struct Span_int FixedVec_as_span_int(struct FixedVec_int* self){
    ...
    cminus_ptr_require_alive(self->data, self->origin_kind, self->origin_stack_id, __FILE__, __LINE__);
    return Span_from_int(self->data, self->len, 0);
}
