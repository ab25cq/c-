...
{    char __cminus_stack_anchor;
    ...
    void* __owned_old0 = holder->values;


    holder->values = values;
    if (__owned_old0 != NULL) {
        Vec_delete_int(__owned_old0);
        cminus_gc_free(__owned_old0);
    }

    void* __owned_old1 = holder->lines;

    holder->lines = lines;
    if (__owned_old1 != NULL) {
        List_delete_int(__owned_old1);
        cminus_gc_free(__owned_old1);
    }

    void* __owned_old2 = holder->lookup;

    holder->lookup = lookup;
    if (__owned_old2 != NULL) {
        Map_delete_int_int(__owned_old2);
    ...
    if (({ struct __CMinusIndex_int __index_result0 = Vec_get_opt_int(holder->values, 1); if (__index_result0.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/field_collection_methods.c-", 28); } __index_result0.payload.Some; }) != 20) {
        if (holder != NULL) {
    ...
    if (({ struct __CMinusIndex_int __index_result1 = List_get_opt_int(holder->lines, 0); if (__index_result1.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/field_collection_methods.c-", 31); } __index_result1.payload.Some; }) != 30) {
        if (holder != NULL) {
            cminus_gc_free(holder);
        }
