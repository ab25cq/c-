...
struct Holder {
    struct Vec_int *values;
    struct List_int *lines;
    struct Map_int_int *lookup;
};
...
static __attribute__((unused)) struct SafeSlots* SafeSlots_clone(struct SafeSlots* self)
{
    struct SafeSlots* copy = cminus_gc_calloc(1, sizeof(struct SafeSlots));
    if (copy == NULL || self == NULL) {
        return copy;
    }
    memcpy(copy->words, self->words, sizeof(copy->words));
    copy->bits = self->bits;
    return copy;
}


int slot_seen(struct SafeSlots *slots)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    slots->bits = Bitmap_from(slots->words, 8);
    Bitmap_set(&(slots->bits), 3);
    __typeof__((Bitmap_test(&(slots->bits), 3))) __cminus_return6 = (Bitmap_test(&(slots->bits), 3));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    struct Holder *holder = cminus_gc_calloc(1, sizeof(struct Holder));
    struct SafeSlots slots = {0};
    memset(&slots, 0, sizeof(slots));

    struct Vec_int *values = Vec_new_int();
    struct List_int *lines = List_new_int();
    struct Map_int_int *lookup = Map_new_int_int();
    struct __CMinusIndex_int found = {0};
    memset(&found, 0, sizeof(found));
    void* __owned_old7 = holder->values;


    holder->values = values;
    if (__owned_old7 != NULL) {
        Vec_delete_int(__owned_old7);
        cminus_gc_free(__owned_old7);
    }

    void* __owned_old8 = holder->lines;

    holder->lines = lines;
    if (__owned_old8 != NULL) {
        List_delete_int(__owned_old8);
        cminus_gc_free(__owned_old8);
    }

    void* __owned_old9 = holder->lookup;

    holder->lookup = lookup;
    if (__owned_old9 != NULL) {
        Map_delete_int_int(__owned_old9);
        cminus_gc_free(__owned_old9);
    }



    Vec_push_int(holder->values, 10);
    Vec_push_int(holder->values, 20);
    List_push_int(holder->lines, 30);
    List_push_int(holder->lines, 40);
    Map_set_int_int(holder->lookup, 7, 70);

    found = Map_get_opt_int_int(holder->lookup, 7);
    if (({ struct __CMinusIndex_int __index_result0 = Vec_get_opt_int(holder->values, 1); if (__index_result0.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/field_collection_methods.c-", 41); } __index_result0.payload.Some; }) != 20) {
        __typeof__((1)) __cminus_return10 = (1);
        if (holder != NULL) {
            cminus_gc_free(holder);
        }

        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return10;
    }
    if (({ struct __CMinusIndex_int __index_result1 = List_get_opt_int(holder->lines, 0); if (__index_result1.tag == __CMinusIndex_int_TAG_None) { cminus_panic("index out of range", "tests/field_collection_methods.c-", 44); } __index_result1.payload.Some; }) != 30) {
    ...
    if (!__CMinusIndex_int_is_Some(&found) || __CMinusIndex_int_get_Some(&found) != 70) {
    ...
    if (!slot_seen(&slots)) {
        __typeof__((4)) __cminus_return13 = (4);
        if (holder != NULL) {
            cminus_gc_free(holder);
        }
