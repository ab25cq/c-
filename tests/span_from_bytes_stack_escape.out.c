...
struct Span_int make_view(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    int raw[2] = { 1, 2 };
    struct Span_int values = ({ cminus_stack_note_caller_range(raw, sizeof(raw)); Span_from_bytes_int(raw, sizeof(raw), 0); });
...
{    char __cminus_stack_anchor;
    ...
    struct Span_int values = make_view();

    __typeof__((Span_get_int(&values, 0))) __cminus_return7 = (Span_get_int(&values, 0));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
