...
{    char __cminus_stack_anchor;
    ...
    map = ({ cminus_stack_note_caller_range(pages.words, sizeof(pages.words)); Bitmap_from_words(pages.words, 1); });
    Bitmap_set(&map, 1);
    __typeof__((Bitmap_test(&map, 1) ? 0 : 1)) __cminus_return6 = (Bitmap_test(&map, 1) ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
