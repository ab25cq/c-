...
struct Volatile_unsigned_int{
    volatile unsigned int* addr;
};
struct StaticCell_int{
    int value;
    int initialized;
};
struct StaticCell_int StaticCell_init_int(int value);
int StaticCell_get_int(struct StaticCell_int* self);
struct Volatile_unsigned_int Volatile_from_addr_unsigned_int(unsigned long addr);
void Volatile_write_unsigned_int(struct Volatile_unsigned_int* self, unsigned int value);
...
{    char __cminus_stack_anchor;
    ...
    unsigned int raw = 0u;
    struct StaticCell_int cell = StaticCell_init_int(5);
    struct Volatile_unsigned_int vol = {0};
    memset(&vol, 0, sizeof(vol));

    int value = {0};
    memset(&value, 0, sizeof(value));
 {
        vol = Volatile_from_addr_unsigned_int((unsigned long)&raw);
    }
    value = StaticCell_get_int(&cell);
    Volatile_write_unsigned_int(&vol, value);
    __typeof__((raw == 5u ? 0 : 1)) __cminus_return6 = (raw == 5u ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return6;
}
struct StaticCell_int StaticCell_init_int(int value){
    struct StaticCell_int out;
...
struct Volatile_unsigned_int Volatile_from_addr_unsigned_int(unsigned long addr){
    struct Volatile_unsigned_int out;
