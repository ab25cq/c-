...
void Ref_set_int(struct Ref_int* self, int value);
...
{    char __cminus_stack_anchor;
    ...
    struct Optional_Ref_int some = Optional_Ref_int_Some(reference);
    struct Optional_Ref_int none = Optional_Ref_int_None();
    struct Ref_int unwrapped = Optional_Ref_int_get_Some(&some);

    if (!Optional_Ref_int_is_Some(&some) || !Optional_Ref_int_is_None(&none) || Ref_get_int(&unwrapped) != 7) {
        __typeof__((1)) __cminus_return6 = (1);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return6;
    }
    Ref_set_int(&unwrapped, 9);
    __typeof__((value == 9 ? 0 : 2)) __cminus_return7 = (value == 9 ? 0 : 2);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
