...
int Atomic_compare_exchange_unsigned_int(struct Atomic_unsigned_int* self, unsigned int expected, unsigned int desired){
    if (self == NULL) {
        cminus_panic("atomic is null", __FILE__, __LINE__);
    }
...
{    char __cminus_stack_anchor;
    ...
    if (old != 0x13u || Atomic_load_unsigned_int(&counter) != 7u) {
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return 3;
    }
    swapped = Atomic_compare_exchange_unsigned_int(&counter, 7u, 9u);
