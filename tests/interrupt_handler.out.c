...
unsigned int IRQ_STATUS_CELL;
struct Register_unsigned_int IRQ_STATUS;__attribute__((interrupt)) 

void timer_irq(void)
{
    Register_set_bits_unsigned_int(&IRQ_STATUS, 0x01u);
}
...
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);
 {
        IRQ_STATUS = Register_from_addr_unsigned_int((unsigned long)&IRQ_STATUS_CELL);
    }
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return 0;
}