...
{    char __cminus_stack_anchor;
    ...
    struct Data *data = cminus_gc_calloc(1, sizeof(struct Data));    void* __owned_old6 = data->text;
    data->text = cminus_string_format("aaa");
    if (__owned_old6 != NULL) {
        cminus_gc_free(__owned_old6);
    }

    void* __owned_old7 = data->text;
    data->text = cminus_string_format("bbb");
    if (__owned_old7 != NULL) {
        cminus_gc_free(__owned_old7);
    }


    if (!cminus_string_eq(data->text, "bbb")) {
