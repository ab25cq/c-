...
int RingBuffer_len_int(struct RingBuffer_int* self);
int RingBuffer_push_int(struct RingBuffer_int* self, int value);
struct __CMinusIndex_int RingBuffer_pop_opt_int(struct RingBuffer_int* self);
...
{    char __cminus_stack_anchor;
    ...
    q = ({ cminus_stack_note_caller_range(storage.values, sizeof(storage.values)); RingBuffer_from_int(storage.values, 3, 0); });
    RingBuffer_push_int(&q, 10);
    RingBuffer_push_int(&q, 20);
    item = RingBuffer_pop_opt_int(&q);
    if (__CMinusIndex_int_is_None(&item) || __CMinusIndex_int_get_Some(&item) != 10 || RingBuffer_len_int(&q) != 1) {
