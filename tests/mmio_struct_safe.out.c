...
struct Uart {
    struct Register_unsigned_int status;
    struct Register_unsigned_int data;
};

static __attribute__((unused)) struct Uart* Uart_clone(struct Uart* self)
{
    struct Uart* copy = cminus_gc_calloc(1, sizeof(struct Uart));
    if (copy == NULL || self == NULL) {
        return copy;
    }
...
{    char __cminus_stack_anchor;
 ...
 {
        uart.status = Register_from_addr_unsigned_int((unsigned long)&status_cell);
        uart.data = Register_from_addr_unsigned_int((unsigned long)&data_cell);
    }

    Register_write_unsigned_int(&(uart.status), 0x01u);
    Register_write_unsigned_int(&(uart.data), 0x41u);
    __typeof__((Register_read_unsigned_int(&(uart.status)) == 1u && Register_read_unsigned_int(&(uart.data)) == 0x41u ? 0 : 1)) __cminus_return6 = (Register_read_unsigned_int(&(uart.status)) == 1u && Register_read_unsigned_int(&(uart.data)) == 0x41u ? 0 : 1);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
