...
#define __CMINUS_DEFAULT_SELECT_RingBuffer_from_int(_1, _2, _3, NAME, ...) NAME
#define __CMINUS_DEFAULT_RingBuffer_from_int_2(data, cap) RingBuffer_from_int(data, cap, 0)
#define __CMINUS_DEFAULT_RingBuffer_from_int_3(data, cap, allow_raw) RingBuffer_from_int(data, cap, allow_raw)
#define RingBuffer_from_int(...) __CMINUS_DEFAULT_SELECT_RingBuffer_from_int(__VA_ARGS__, __CMINUS_DEFAULT_RingBuffer_from_int_3, __CMINUS_DEFAULT_RingBuffer_from_int_2)(__VA_ARGS__)
struct RingBuffer_int RingBuffer_from_int(int* data, int cap, int allow_raw);
#define __CMINUS_DEFAULT_SELECT_RingBuffer_from_bytes_int(_1, _2, _3, NAME, ...) NAME
#define __CMINUS_DEFAULT_RingBuffer_from_bytes_int_2(data, bytes) RingBuffer_from_bytes_int(data, bytes, 0)
#define __CMINUS_DEFAULT_RingBuffer_from_bytes_int_3(data, bytes, allow_raw) RingBuffer_from_bytes_int(data, bytes, allow_raw)
#define RingBuffer_from_bytes_int(...) __CMINUS_DEFAULT_SELECT_RingBuffer_from_bytes_int(__VA_ARGS__, __CMINUS_DEFAULT_RingBuffer_from_bytes_int_3, __CMINUS_DEFAULT_RingBuffer_from_bytes_int_2)(__VA_ARGS__)
struct RingBuffer_int RingBuffer_from_bytes_int(int* data, int bytes, int allow_raw);
int RingBuffer_push_int(struct RingBuffer_int* self, int value);
struct __CMinusIndex_int RingBuffer_peek_opt_int(struct RingBuffer_int* self);
...
struct RingBuffer_int make_ring(void)
{    char __cminus_stack_anchor;
    size_t __cminus_stack_id = cminus_stack_enter_impl(__FILE__, __LINE__, &__cminus_stack_anchor);

    int raw[2] = { 1, 2 };
    struct RingBuffer_int values = ({ cminus_stack_note_caller_range(raw, sizeof(raw)); RingBuffer_from_bytes_int(raw, sizeof(raw), 0); });

    RingBuffer_push_int(&values, 1);
    __typeof__((values)) __cminus_return6 = (values);
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
...
{    char __cminus_stack_anchor;
    ...
    struct RingBuffer_int values = make_ring();
    struct __CMinusIndex_int value = RingBuffer_peek_opt_int(&values);

    if (__CMinusIndex_int_is_None(&value)) {
        __typeof__((1)) __cminus_return7 = (1);
        cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
        return __cminus_return7;
    }
    __typeof__((__CMinusIndex_int_get_Some(&value))) __cminus_return8 = (__CMinusIndex_int_get_Some(&value));
    cminus_stack_leave_impl(__cminus_stack_id, __FILE__, __LINE__);
    return __cminus_return8;
}
#undef RingBuffer_from_int
struct RingBuffer_int RingBuffer_from_int(int* data, int cap, int allow_raw){
    struct RingBuffer_int out;
...
struct __CMinusIndex_int RingBuffer_peek_opt_int(struct RingBuffer_int* self){
    ...
    return __CMinusIndex_int_Some(self->data[self->head]);
}
