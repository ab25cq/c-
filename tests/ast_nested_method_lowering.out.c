...
{    char __cminus_stack_anchor;
    ...
    char* text;
    text = cminus_string_format("abc");
    __typeof__((cminus_string_eq(text, cminus_string_starts_with(text, "a") ? "abc" : "") ? 0 : 1)) __cminus_return6 = (cminus_string_eq(text, cminus_string_starts_with(text, "a") ? "abc" : "") ? 0 : 1);
    if (text != NULL) {
        cminus_gc_free(text);
    }
